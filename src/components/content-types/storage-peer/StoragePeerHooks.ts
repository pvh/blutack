...
import { useRepo, useDocument } from "automerge-repo-react-hooks";
...
export function useStoragePeer(
  ...
  FreezeObject<StoragePeerDoc> | null,
  boolean | null,
  RegistrationFn,
  UnregistrationFn
] {
  ...
  const currentWorkspace =
    workspaceUrlsContext && workspaceUrlsContext.workspaceUrls[0];
  const workspaceUrl =
    currentWorkspace && parseDocumentLink(currentWorkspace).hypermergeUrl;
  ...
  const register = useCallback(async () => {
    ...
    const encryptionKey = await getVerifiedEncryptionKey(
      repo,
      storagePeerUrl,
      doc
    );
    ...
    const sealedWorkspace = await repo.crypto.sealedBox(
      encryptionKey,
      workspaceUrl
    );
    changeDoc((doc) => {
